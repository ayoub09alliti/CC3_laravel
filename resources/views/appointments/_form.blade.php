...
<div class="grid gap-6 md:grid-cols-2">
    ...
    <div>
        <label for="doctor_id" class="mb-1 block text-sm font-medium text-slate-700">Medecin</label>
    ...
    <div>
        <label for="time" class="mb-1 block text-sm font-medium text-slate-700">Heure</label>
    ...
    <div>
        ...
        <select id="status" name="status" required class="w-full rounded-md border border-slate-300 bg-white px-3 py-2 focus:border-sky-500 focus:outline-none">
            @foreach (['pending' => 'En attente', 'confirmed' => 'Confirme', 'cancelled' => 'Annule'] as $value => $label)
...
<div class="mt-8 flex flex-col gap-3 sm:flex-row">
    <button type="submit" class="rounded-md bg-slate-900 px-4 py-2 font-medium text-white hover:bg-slate-700">
        {{ $isEdit ? 'Mettre a jour' : 'Creer le rendez-vous' }}
    </button>
