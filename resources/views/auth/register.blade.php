@extends('layouts.app', ['title' => __('Inscription')])

@section('content')
    <div class="mx-auto max-w-md rounded-lg bg-white p-8 shadow-sm ring-1 ring-slate-200">
        <h1 class="text-2xl font-semibold">{{ __('Inscription') }}</h1>
        <p class="mt-2 text-sm text-slate-600">{{ __('La creation publique d un compte cree un profil patient.') }}</p>

        <form method="POST" action="{{ route('register.store') }}" class="mt-6 space-y-4">
            @csrf

            <div>
                <label for="name" class="mb-1 block text-sm font-medium text-slate-700">{{ __('Nom complet') }}</label>
            ...
            <div>
                <label for="password" class="mb-1 block text-sm font-medium text-slate-700">{{ __('Mot de passe') }}</label>
            ...
            <div>
                <label for="password_confirmation" class="mb-1 block text-sm font-medium text-slate-700">{{ __('Confirmation') }}</label>
        ...
        <p class="mt-6 text-sm text-slate-600">
            {{ __('Deja inscrit ?') }}
            <a href="{{ route('login') }}" class="font-medium text-slate-900 hover:text-slate-700">{{ __('Se connecter') }}</a>
        </p>
    </div>
@endsection
