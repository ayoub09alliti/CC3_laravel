@extends('layouts.app', ['title' => __('Connexion')])

@section('content')
    <div class="mx-auto max-w-md rounded-lg bg-white p-8 shadow-sm ring-1 ring-slate-200">
        <h1 class="text-2xl font-semibold">{{ __('Connexion') }}</h1>
        <p class="mt-2 text-sm text-slate-600">{{ __('Connectez-vous avec votre compte pour acceder a votre espace.') }}</p>
        ...
        <p class="mt-6 text-sm text-slate-600">
            ...
            <a href="{{ route('register') }}" class="font-medium text-sky-700 hover:text-sky-600">{{ __('Creer un compte patient') }}</a>
        </p>
    </div>
@endsection
