@extends('layouts.app', ['title' => __('Tableau de bord medecin')])

@section('content')
    <section class="mx-auto max-w-4xl rounded-lg bg-white p-8 shadow-sm ring-1 ring-slate-200">
        <div class="flex flex-col gap-6 md:flex-row md:items-start md:justify-between">
            <div>
                <p class="text-sm font-medium uppercase tracking-wide text-emerald-700">{{ __('Medecin') }}</p>
        ...
        <dl class="mt-8 grid gap-4 md:grid-cols-3">
            ...
            <div class="rounded-lg bg-slate-50 p-4">
                <dt class="text-sm text-slate-500">{{ __('Specialite') }}</dt>
                <dd class="mt-1 font-medium">{{ auth()->user()->specialty ?: __('Non renseignee') }}</dd>
            </div>
