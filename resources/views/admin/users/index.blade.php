...
@section('content')
    @php
        ...
        $resourceLabel = $isDoctor ? 'medecin' : 'patient';
        $resourceLabelPlural = $isDoctor ? 'medecins' : 'patients';
        $accentClass = $isDoctor ? 'emerald' : 'sky';
    @endphp
    ...
    <section class="mx-auto max-w-7xl space-y-6">
        <div class="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div>
                ...
                <p class="mt-2 text-sm text-slate-600">Creation, modification et suppression des {{ $resourceLabelPlural }} depuis l'espace admin.</p>
            </div>
        ...
        <div class="grid gap-4 md:grid-cols-3">
            ...
            <div class="rounded-lg bg-white p-4 shadow-sm ring-1 ring-slate-200">
                <p class="text-sm text-slate-500">Page courante</p>
                <p class="mt-2 text-2xl font-semibold">{{ $users->currentPage() }}</p>
            </div>
            <div class="rounded-lg bg-white p-4 shadow-sm ring-1 ring-slate-200">
                <p class="text-sm text-slate-500">Affiches</p>
                    ...
                    <tbody class="divide-y divide-slate-100 text-sm text-slate-700">
                        @forelse ($users as $user)
                            <tr class="hover:bg-slate-50">
                                ...
                                @if ($isDoctor)
                                    <td class="px-4 py-3">{{ $user->specialty ?: '-' }}</td>
                                @endif
                                <td class="px-4 py-3">
        ...
        <div id="delete-user-modal-{{ $user->id }}" class="fixed inset-0 z-50 hidden items-center justify-center bg-slate-950/50 px-4" data-modal>
            <div class="w-full max-w-md rounded-lg bg-white p-6 shadow-xl">
                <div class="flex items-start justify-between gap-4">
                    <div>
                        <h2 class="text-xl font-semibold text-slate-900">Supprimer {{ $resourceLabel }}</h2>
                        <p class="mt-2 text-sm text-slate-600">Cette action supprimera le compte de {{ $user->name }}.</p>
                    </div>
                    <button type="button" class="rounded-md p-2 text-slate-500 hover:bg-slate-100" data-modal-close="delete-user-modal-{{ $user->id }}">
                        X
                    </button>
                </div>
