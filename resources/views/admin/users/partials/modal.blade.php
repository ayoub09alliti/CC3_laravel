    ...
    <div class="w-full max-w-2xl rounded-lg bg-white p-6 shadow-xl">
        <div class="flex items-start justify-between gap-4">
            <div>
                ...
                <p class="mt-2 text-sm text-slate-600">
                    {{ $isDoctor ? 'Renseignez les informations du medecin.' : 'Renseignez les informations du patient.' }}
                </p>
            </div>
            <button type="button" class="rounded-md p-2 text-slate-500 hover:bg-slate-100" data-modal-close="{{ $modalId }}">
                X
            </button>
        </div>
        ...
        <form method="POST" action="{{ $action }}" class="mt-6 space-y-4">
            ...
            <div class="grid gap-4 md:grid-cols-2">
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700" for="{{ $modalId }}-name">Nom</label>
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700" for="{{ $modalId }}-email">Email</label>
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700" for="{{ $modalId }}-phone">Telephone</label>
                ...
                @if ($isDoctor)
                    <div>
                        <label class="mb-1 block text-sm font-medium text-slate-700" for="{{ $modalId }}-specialty">Specialite</label>
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700" for="{{ $modalId }}-password">Mot de passe {{ $isEdit ? '(laisser vide pour conserver)' : '' }}</label>
                ...
                <div>
                    <label class="mb-1 block text-sm font-medium text-slate-700" for="{{ $modalId }}-password_confirmation">Confirmation</label>
            ...
            <div>
                <label class="mb-1 block text-sm font-medium text-slate-700" for="{{ $modalId }}-bio">Bio</label>
